...
- **全程成本可追踪**：0.08s 坏依赖快筛**零成本**；深度评测一次约 $2（国产模型，约为 Claude 的 1/22）——CI 门禁测得起
...
### ⚡ 最快方式：一键启动
```bash
bash setup.sh   # 自动建 venv → 装依赖 → 检查 .env → seed → 启动控制台 http://localhost:8000
```
下面三步是手动分解版。

### 第 0 步：先看效果（零依赖）
仓库已内置 **5 个真实评测 case 的结果**（case_01~04 + case_real），不装任何环境也能看：
```text
case_01/eval_run_result.json   # 运行结果与评级
case_01/eval_accuracy.json     # 业务准确性评测
case_01/eval_quality.json      # 四大工程支柱评测
```
（case_02 / case_03 / case_04 / case_real 同理。）想自己上手跑，再走下面三步。
